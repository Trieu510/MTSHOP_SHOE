...
<div class="container-fluid flash-sale-edit-container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="mt-4 text-dark fw-bold">Chỉnh sửa Flash Sale</h1>
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <div class="mb-3 form-group">
                    ...
                    <select name="applies_to" id="applies_to" class="form-select @error('applies_to') is-invalid @enderror" onchange="toggleTargetSelect()">
                        <option value="all" {{ old('applies_to', $flashSale->applies_to) == 'all' ? 'selected' : '' }}>Tất cả sản phẩm</option>
                        <option value="category" {{ old('applies_to', $flashSale->applies_to) == 'category' ? 'selected' : '' }}>Theo danh mục</option>
                        <option value="product" {{ old('applies_to', $flashSale->applies_to) == 'product' ? 'selected' : '' }}>Sản phẩm cụ thể</option>
                    </select>
                    @error('applies_to')
                ...
                {{-- Chọn danh mục --}}
                <div class="mb-3 form-group" id="category_select" style="display: none;">
                    <label for="category_id" class="form-label">Chọn danh mục</label>
                    <select name="category_id" class="form-select @error('category_id') is-invalid @enderror">
                        <option value="">-- Chọn danh mục --</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ old('category_id', $flashSale->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- Chọn sản phẩm --}}
                <div class="mb-3 form-group" id="product_select" style="display: none;">
                ...
                <div class="mb-3 form-group">
                    ...
                    <input type="number" name="discount_amount" id="discount_amount" class="form-control @error('discount_amount') is-invalid @enderror" step="1000" min="0"
                           value="{{ old('discount_amount', $flashSale->discount_amount) }}">
...
<script>
    function toggleTargetSelect() {
        const applyTo = document.getElementById('applies_to').value;
        document.getElementById('category_select').style.display = applyTo === 'category' ? 'block' : 'none';
        document.getElementById('product_select').style.display = applyTo === 'product' ? 'block' : 'none';
    }
